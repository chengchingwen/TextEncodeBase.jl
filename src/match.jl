...
@inline tokenize(t::MatchTokenization, s::SentenceStage, (istoken, x)) = istoken ? Word(x, s.meta) : SubSentence(x, s.meta)
...
function tokenize(::IndexedMatchTokenization, s::SentenceStage, (offset, (istoken, x), rsibling))
    meta = updatemeta(s.meta, (offset = offset, rsibling = rsibling))
    return istoken ? Word(x, meta) : SubSentence(x, meta)
end
...
function tokenize(::IndexedMatchTokenization, s::SubSentenceStage, (i, x))
    ...
    return Word(x, updatemeta(meta, (token_id = i+offset,)))
end
...
function tokenize(::IndexedMatchTokenization, x::TokenStage)
    if haskey(x.meta, :offset) && haskey(x.meta, :rsibling)
        ...
        return [setmeta(x, updatemeta(meta, (token_id = cid,)))]
    end
    return [x]
end
